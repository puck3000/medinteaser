<template>
  <h1 :style="{ '--color': color }"  @click="randomColor">
    <svg
      id="Ebene_1"
    ...
    >
      <switch>
        <g>
          ...
          />
          <g class="lighten" :style="{ '--darkenedColor': darkenedColor }">
            <path
              class="st0"
...
export default {
    data() {
        return {
            random: '34',
            color: 'hsl(34,80%,60%)',
            darkenedColor: 'hsl(34,60%,40%)',
            percentages: '80%,60%',
            darkenedPercentages: '60%,40%'
        }
    },
    methods: {
        randomColor(){
            let random = Math.round(Math.random() * 360);
            this.color = 'hsl(' + random  + ',80%,60%)';
            this.darkenedColor = 'hsl(' + random + ',40%,60%)';
        }
